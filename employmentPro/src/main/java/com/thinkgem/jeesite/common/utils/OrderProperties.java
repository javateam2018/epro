...
public class OrderProperties extends Properties {
	 ...
	 * 半字节转换为十六进制字符
	 * @param nibble the nibble to convert.(半字节半字节转换)
	 */
	...
	//从BufferedReader:字符输入流中读取文本，缓冲各个字符，从而实现字符、数组和行的高效读取。 
		BufferedReader in;
		...
		while (true) {
			...
			if (line.length() > 0) {
				...
				if ((firstChar != '#') && (firstChar != '!')) {
					...
					// Find separation between key and value：找到键和值之间的分离
					int separatorIndex;
					...
					// Skip over whitespace after key if any：后跳过空格键
					int valueIndex;
					...
					// Convert then store key and value:然后转换存储键和值
					key = loadConvert(key);
	 ...
	 * 转换编码& # 92;uxxxx 采用双字节对字符进行编码字符和改变特别的保存字符原来的形式
	 */
